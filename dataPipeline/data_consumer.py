...
class DataConsumer(SequentialParallelFramework.Consumer):
	...
	self.new_index_filename_ = self.cfg_.get("index", "index_filename")
	self.logger_.debug("read config '[index]index_filename':" + self.new_index_filename_)
	...
	tmp_dir = self.cfg_.get("index", "tmp_dir")
	self.logger_.debug("read config '[index]tmp_dir':" + tmp_dir)
	self.data_dir_ = self.cfg_.get("index", "data_dir")
	self.logger_.debug("read config '[index]data_dir':" + self.data_dir_)
	...
	self.index_merger_ = index_merger.IndexMerger(tmp_dir, False)
	...
	merge_index_filepath = "%s/%s" % (self.data_dir_, self.merge_index_filename_)
	new_index_filepath = "%s/%s" % (self.data_dir_, self.new_index_filename_)
	...
	if filepath:
            ...
            target_filepath = "%s/%s-%s" % (self.data_dir_, self.merge_name_prefix_, data_datetime)
            shutil.move(filepath, target_filepath)
	    tmp_index_path = "%s/.%s.index.tmp" % (self.data_dir_, self.merge_name_prefix_)
	    f = open(tmp_index_path, "w")
	    f.write("%s\n" % target_filepath)
	    f.close()
	    target_mergefile_path = "%s/%s" % (self.data_dir_, self.merge_index_filename_)
